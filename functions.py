...
def headlines_df(table):
	...
	for i in table.findAll('tr'):
		...
		if len(date_scrape) == 1:
			time = date_scrape[0]
			date = today  # Use today's date if only time is provided
		else:
			date = date_scrape[0]
			time = date_scrape[1]
		...
		if 'date' in locals() and 'time' in locals() and text:
	...
	try:
		news_df['datetime'] = pd.to_datetime(news_df['date'] + ' ' + news_df['time'], errors='coerce')
		# Fill any NaT values with current time
		news_df['datetime'] = news_df['datetime'].fillna(pd.Timestamp.now())
	except Exception as e:
		print(f"Error parsing datetime: {e}")
		# Fallback to current datetime
		news_df['datetime'] = pd.Timestamp.now()
...
classifier = TextClassifier.load(
	'resources/taggers/question-classification-with-transformer/final-model.pt'
)
...
def flair_sentiment_score(news_df):
	...
	for s in news_df['news'].values.tolist():
		...
		# Print sample score structure for debugging
		if len(scores) == 1:
			print("Sample score structure:", scores[0])

	for s in scores:
		# Check structure and extract sentiment value and confidence
		try:
			if 'all labels' in s:
				# Original expected format
				rawscores.append(int(s['all labels'][0]['value']) * s['all labels'][0]['confidence'])
			elif 'labels' in s:
				# Alternative format - adapt based on actual structure
				label = s['labels'][0]
				# Check if value exists or use score
				value = int(label.get('value', label.get('score', 0)))
				confidence = label.get('confidence', 1.0) 
				rawscores.append(value * confidence)
			else:
				# Fallback
				print(f"Unknown score format: {s}")
				rawscores.append(0)
		except Exception as e:
			print(f"Error processing score: {e}")
			rawscores.append(0)

	# Ensure rawscores has the same length as the dataframe
	if len(rawscores) < len(news_df):
		rawscores.extend([0] * (len(news_df) - len(rawscores)))
...
def graph_tree(df):
	fig = px.treemap(
		df,
		path=[px.Constant('Sectors'), 'Sector', 'Industry', df.index],
		values='Market Cap',
		color='Sentiment',
